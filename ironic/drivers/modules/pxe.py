...
class PXEDeploy(base.DeployInterface):
    ...
    def validate(self, task):
        ...
        if CONF.pxe.ipxe_enabled:
            ...
            if pxe_utils.get_node_capability(task.node, 'boot_mode') == 'uefi':
                LOG.exception(_LE(
                  "UEFI boot mode is not supported with iPXE boot enabled."))
                raise exception.ConfigInvalid(_(
    ...
    def deploy(self, task):
        ...
        # Under UEFI boot mode, setting of boot device may differ between
        # different machines. IPMITool does not work for setting boot devices
        # in UEFI mode for certain machines.
        # Expected ipmitool failure for uefi boot mode. Logging a message to
        # set the boot device manullay and continue with deploy.
        try:
