...
BOOT_MODE_GENERIC_TO_ILO = {'bios': 'legacy', 'BIOS': 'LEGACY'}
...
def set_boot_mode(node, boot_mode):
    ...
    if p_boot_mode.lower().replace('legacy',
                 BOOT_MODE_ILO_TO_GENERIC['legacy']) == boot_mode:
    ...
    try:
        ilo_object.set_pending_boot_mode(
                        boot_mode.replace('bios',
                        BOOT_MODE_GENERIC_TO_ILO['bios']).upper())
    except ilo_client.IloError as ilo_exception:
...
def update_boot_mode_capability(task):
    ...
    driver_utils.add_node_capability(task, 'boot_mode',
                         p_boot_mode.replace('LEGACY',
                         BOOT_MODE_ILO_TO_GENERIC['LEGACY']).lower())
